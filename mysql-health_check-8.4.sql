...
SELECT * FROM (
    SELECT '<div id="h_2" class="sub-title">1.2 Host Summary by File IO</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$host_summary_by_file_io` ORDER BY x$host_summary_by_file_io.io_latency DESC</pre><table><tr><th>Host</th><th>IOs</th><th>IO Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="h_3" class="sub-title">1.3 Host Summary by File IO Type</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$host_summary_by_file_io_type` ORDER BY x$host_summary_by_file_io_type.host, x$host_summary_by_file_io_type.total_latency DESC</pre><table><tr><th>Host</th><th>Event Name</th><th>Total</th><th>Total Latency</th><th>Max Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="h_4" class="sub-title">1.4 Host Summary by Stages</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$host_summary_by_stages` ORDER BY x$host_summary_by_stages.host, x$host_summary_by_stages.total_latency DESC</pre><table><tr><th>Host</th><th>Event Name</th><th>Total</th><th>Total Latency</th><th>Avg Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="h_5" class="sub-title">1.5 Host Summary by Statement Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$host_summary_by_statement_latency` ORDER BY x$host_summary_by_statement_latency.total_latency DESC</pre><table><tr><th>Host</th><th>Total</th><th>Total Latency</th><th>Max Latency</th><th>Lock Latency</th><th>CPU Latency</th><th>Rows Sent</th><th>Rows Examined</th><th>Rows Affected</th><th>Full Scans</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="h_6" class="sub-title">1.6 Host Summary by Statement Type</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$host_summary_by_statement_type` ORDER BY x$host_summary_by_statement_type.host, x$host_summary_by_statement_type.total_latency DESC</pre><table><tr><th>Host</th><th>Statement</th><th>Total</th><th>Total Latency</th><th>Max Latency</th><th>Lock Latency</th><th>CPU Latency</th><th>Rows Sent</th><th>Rows Examined</th><th>Rows Affected</th><th>Full Scans</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="io_1" class="sub-title">2.1 IO by Thread by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$io_by_thread_by_latency` ORDER BY x$io_by_thread_by_latency.total_latency DESC</pre><table><tr><th>User</th><th>Total</th><th>Total Latency</th><th>Min Latency</th><th>Avg Latency</th><th>Max Latency</th><th>Thread ID</th><th>Processlist ID</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="io_2" class="sub-title">2.2 IO Global by File by Bytes</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$io_global_by_file_by_bytes` ORDER BY x$io_global_by_file_by_bytes.total DESC</pre><table><tr><th>File</th><th>Count Read</th><th>Total Read</th><th>Avg Read</th><th>Count Write</th><th>Total Written</th><th>Avg Write</th><th>Total</th><th>Write Pct</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="io_3" class="sub-title">2.3 IO Global by File by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$io_global_by_file_by_latency` ORDER BY x$io_global_by_file_by_latency.total_latency DESC</pre><table><tr><th>File</th><th>Total</th><th>Total Latency</th><th>Count Read</th><th>Read Latency</th><th>Count Write</th><th>Write Latency</th><th>Count Misc</th><th>Misc Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="io_5" class="sub-title">2.5 IO Global by Wait by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$io_global_by_wait_by_latency` ORDER BY x$io_global_by_wait_by_latency.total_latency DESC</pre><table><tr><th>Event Name</th><th>Total</th><th>Total Latency</th><th>Avg Latency</th><th>Max Latency</th><th>Read Latency</th><th>Write Latency</th><th>Misc Latency</th><th>Count Read</th><th>Total Read</th><th>Avg Read</th><th>Count Write</th><th>Total Written</th><th>Avg Written</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="u_2" class="sub-title">3.2 User Summary by File IO</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$user_summary_by_file_io` ORDER BY x$user_summary_by_file_io.io_latency DESC</pre><table><tr><th>User</th><th>IOs</th><th>IO Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="u_3" class="sub-title">3.3 User Summary by File IO Type</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$user_summary_by_file_io_type` ORDER BY x$user_summary_by_file_io_type.user, x$user_summary_by_file_io_type.latency DESC</pre><table><tr><th>User</th><th>Event Name</th><th>Total</th><th>Latency</th><th>Max Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="u_4" class="sub-title">3.4 User Summary by Stages</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$user_summary_by_stages` ORDER BY x$user_summary_by_stages.user, x$user_summary_by_stages.total_latency DESC</pre><table><tr><th>User</th><th>Event Name</th><th>Total</th><th>Total Latency</th><th>Avg Latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="u_5" class="sub-title">3.5 User Summary by Statement Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$user_summary_by_statement_latency` ORDER BY x$user_summary_by_statement_latency.total_latency DESC</pre><table><tr><th>User</th><th>Total</th><th>Total Latency</th><th>Max Latency</th><th>Lock Latency</th><th>CPU Latency</th><th>Rows Sent</th><th>Rows Examined</th><th>Rows Affected</th><th>Full Scans</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="u_6" class="sub-title">3.6 User Summary by Statement Type</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$user_summary_by_statement_type` ORDER BY x$user_summary_by_statement_type.user, x$user_summary_by_statement_type.total_latency DESC</pre><table><tr><th>User</th><th>Statement</th><th>Total</th><th>Total Latency</th><th>Max Latency</th><th>Lock Latency</th><th>CPU Latency</th><th>Rows Sent</th><th>Rows Examined</th><th>Rows Affected</th><th>Full Scans</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="m_1" class="sub-title">4.1 Memory by Host</div><pre class="query-sql"># Query: select * from sys.memory_by_host_by_current_bytes</pre><table><tr><th>host</th><th>current_count_used</th><th>current_allocated</th><th>current_avg_alloc</th><th>current_max_alloc</th><th>total_allocated</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="m_2" class="sub-title">4.2 Memory by Thread</div><pre class="query-sql"># Query: select * from sys.memory_by_thread_by_current_bytes</pre><table><tr><th>thread_id</th><th>user</th><th>current_count_used</th><th>current_allocated</th><th>current_avg_alloc</th><th>current_max_alloc</th><th>total_allocated</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="m_3" class="sub-title">4.3 Memory by User</div><pre class="query-sql"># Query: select * from sys.memory_by_user_by_current_bytes</pre><table><tr><th>user</th><th>current_count_used</th><th>current_allocated</th><th>current_avg_alloc</th><th>current_max_alloc</th><th>total_allocated</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="m_4" class="sub-title">4.4 Memory Global Summary</div><pre class="query-sql"># Query: select * from sys.memory_global_by_current_bytes</pre><table><tr><th>event_name</th><th>current_count</th><th>current_alloc</th><th>current_avg_alloc</th><th>high_count</th><th>high_alloc</th><th>high_avg_alloc</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="w_1" class="sub-title">5.1 Waits Global by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$waits_global_by_latency` ORDER BY x$waits_global_by_latency.total_latency DESC</pre><table><tr><th>events</th><th>total</th><th>total_latency</th><th>avg_latency</th><th>max_latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="w_2" class="sub-title">5.2 Waits by User by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$waits_by_user_by_latency` ORDER BY x$waits_by_user_by_latency.user, x$waits_by_user_by_latency.total_latency DESC</pre><table><tr><th>user</th><th>event</th><th>total</th><th>total_latency</th><th>avg_latency</th><th>max_latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="w_3" class="sub-title">5.3 Waits by Host by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$waits_by_host_by_latency` ORDER BY x$waits_by_host_by_latency.host, x$waits_by_host_by_latency.total_latency DESC</pre><table><tr><th>host</th><th>event</th><th>total</th><th>total_latency</th><th>avg_latency</th><th>max_latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="w_4" class="sub-title">5.4 Wait Classes Global by Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$wait_classes_global_by_latency` ORDER BY x$wait_classes_global_by_latency.total_latency DESC</pre><table><tr><th>event_class</th><th>total</th><th>total_latency</th><th>min_latency</th><th>avg_latency</th><th>max_latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="w_5" class="sub-title">5.5 Wait Classes Global by Avg Latency</div><pre class="query-sql"># Query: SELECT * FROM `sys`.`x$wait_classes_global_by_avg_latency` ORDER BY IFNULL(x$wait_classes_global_by_avg_latency.total_latency / NULLIF(x$wait_classes_global_by_avg_latency.total, 0), 0) DESC</pre><table><tr><th>event_class</th><th>total</th><th>total_latency</th><th>min_latency</th><th>avg_latency</th><th>max_latency</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="idx_3" class="sub-title">6.3 Schema Unused Indexes</div><pre class="query-sql"># Query: SELECT * FROM sys.schema_unused_indexes where object_schema not in (''performance_schema'',''information_chema'',''mysql'',''sys'');</pre><table><tr><th>object_schema</th><th>object_name</th><th>index_name</th></tr>'
...
SELECT * FROM (
    SELECT '<div id="idx_4" class="sub-title">6.4 Low Selectivity Secondary Indexes</div><pre class="query-sql"># Query: SELECT i.database_name AS db_name,i.table_name AS table_name,i.index_name AS index_name,i.stat_value AS def_Rows,t.n_rows AS total_rows, ROUND(((i.stat_value / IFNULL(IF(t.n_rows < i.stat_value,i.stat_value,t.n_rows),0.01))),2) AS sel_persent FROM mysql.innodb_index_stats i INNER JOIN mysql.innodb_table_stats t ON i.database_name = t.database_name AND i.table_name = t.table_name WHERE i.index_name != ''PRIMARY'' AND i.stat_name LIKE ''%n_diff_pfx%'' AND ROUND(((i.stat_value / IFNULL(IF(t.n_rows < i.stat_value,i.stat_value,t.n_rows),0.01))),2) < 0.1;</pre><table><tr><th>db_name</th><th>table_name</th><th>index_name</th><th>def_Rows</th><th>total_rows</th><th>sel_persent</th></tr>'
